...
function SkillsSection() {
    return (
        <section className="flex flex-col gap-9 bg-zinc-950 px-8 py-16">
            ...
            <div className="flex flex-wrap items-center justify-center gap-5 py-2">
                {icons.map((i) => (
                    <div
                        key={i.id}
                        className="flex h-24 w-24 flex-col items-center justify-center gap-2 rounded-4xl rounded-tl-none border-2 border-zinc-300"
                    >
                        <img src={i.src} alt={i.name} className="h-11 w-11" />
